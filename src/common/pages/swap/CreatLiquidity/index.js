    ...
    return (
      <div className="panel">
        ...
        <div className="add-liquidity-content">
          <CoinDropdown token={baseToken} onChange={this.changeBase} loadCoinList={this.getTokens} />
          <div className="sep add-sep"></div>
          <CoinDropdown token={targetToken} onChange={this.changeTarget} loadCoinList={this.getTokens} />
          {error && <div className="error-tip error-tip-link" onClick={this.addLiquidity}>{toLocale('Error')}：{toLocale('Existed Pool')}</div>}
          <div className="btn-wrap">
            {disabled ? <div className="btn disabled">{toLocale('Create Liquidity')}</div>:<div className="btn" onClick={this.confirm}>{toLocale('Create Liquidity')}</div>}
          </div>
        </div>
      </div>
    );
