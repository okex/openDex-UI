      ...
      return (
        <td width={d.width} key={index}>
          <div className={classNames({'head-sort':d.canSort})}>
            ...
            {d.canSort && 
            <div>
              <i className={classNames('up',{active:this.isFieldSort(d,ASC)})} onClick={() => this.sortField(d,ASC)}/><i className={classNames('down',{active:this.isFieldSort(d,DESC)})} onClick={() => this.sortField(d,DESC)}/>
            </div>
            }
          </div>
        </td>
    ...
    return (
      <table>
        <tbody>
          ...
          {data.map((d, index) => <DataRow key={index} data={d} columns={columns}/>)}
        </tbody>
      </table>
    );
  }
