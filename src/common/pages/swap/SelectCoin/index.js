  ...
  render() {
    ...
    return this.init ? (
      <div className="panel-coin-search">
        <div className="search-wrap iconfont"><input placeholder={toLocale('Search name')} defaultValue={search} onInput={this.search}/></div>
        <div className="search-content-wrap">
          <div className="search-content-items">
            {list.length ? list.map((d, index) => <div className="search-content-item" key={index} onClick={() => this.select(d)}>
              <div className="name">
