  ...
  fetchAccounts = () => {
    ...
    const fetchContract = () =>
      new Promise((resolve) => {
        ...
        Promise.allSettled(contractPromiseList)
          .then((res) => {
            let response = [];
            res.forEach(
              (item) => item.status === 'fulfilled' && response.push(item.value)
            );
            resolve(response);
          })
          .catch(() => resolve([]));
    ...
    fetchContract()
      ...
      .then(([currencies, tokenMap]) => {
        ...
        let oktIndex = currencies.findIndex(
          (it) => it.symbol === env.envConfig.token.base
        );
          ...
          columns={assetsUtil.accountsCols(
            {
              ...
              moreOperationsChange: this.moreOperationsChange,
            },
            { valuationUnit }
          )}
