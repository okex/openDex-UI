  ...
  render() {
    ...
    return (
      <div className="report-list-container">
        ...
        <ul>
          {dataSource.map(
            ({ title, publishTime: createTime, accessory }, index) => {
              return (
                <li key={index}>
                  <a href={accessory} target="_blank" rel="noopener noreferrer">
                    <span title={title}>
                      <label>{toLocale('spot.kline.tabs[2]')}</label>
        ...
        {!loading && dataSource.length > 0 && (
          <a className="more" href={`/project/${projectId}?tab=1`}>
            {toLocale('spot.kline.tabs[2].more')}&nbsp;&gt;
          </a>
        )}
      </div>
    );
