...
class DexDesktopInput extends Component {
  ...
  render() {
    const { value, onChange, hint, label, multiple } = this.props;
    return (
      <div className="dex-desktop-input-container">
        <div className="desktop-input-title-row">
          <label className="desktop-input-label" htmlFor="">
            {label}
          </label>
          {hint && (
        ...
        {multiple ? (
          ...
          <input className="desktop-input" value={value} onChange={onChange} />
        )}
      </div>
    );
  }
