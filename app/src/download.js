...
module.exports = () => {
  ...
  request(RELEASE_URL).then((response) => {
    ...
    const okexchaindObj = data.assets.filter((d) =>
      d.name.includes(assetType)
    )[0];
    ...
    const start = async (isRedownload = false) => {
      ...
      if (!fs.existsSync(lastVersionDirectory)) {
        ...
        if (
          !releaseTag ||
          !fs.existsSync(`${lastVersionDirectory}/okexchaind`)
        ) {
          isOKExChaindDownload = false;
        }
        if (
          !cliReleaseTag ||
          !fs.existsSync(`${lastVersionDirectory}/okexchaincli`)
        ) {
          isOKExChaindDownload = false;
        }
      }
